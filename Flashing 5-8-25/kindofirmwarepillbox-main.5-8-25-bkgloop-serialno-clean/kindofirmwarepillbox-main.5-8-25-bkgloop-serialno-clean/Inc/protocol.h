...
// Limit switch     0x5x    7bytes
  ...
  PILLDR_CLS      = 0x53,
  PILLBX_DETE     = 0x54,   //    detect the pillbx is here or not, hall sensor
  
  FRNTDR_BUTT     = 0x55,   //    Door gate button
  FRNTDR_CLS      = 0x56,   //    Door gate closed ?
  
  ARM_X_STP1      = 0x57,   //    top arm X-dir limit switch 1
  ARM_X_STP2      = 0x58,   //    top arm X-dir limit switch 2
  ARM_Z_STP1      = 0x59,   //    top arm Z-dir limit switch 1
  ARM_Z_STP2      = 0x5a,   //    top arm Z-dir limit switch 2
  
  PROT_ENCI       = 0x5b,   //    Pillbox rotary encoder 
  PROT_ENCZ       = 0x5c,   //    External encoder
  CROT_ENCI       = 0x5d,   //    Canister rotary encoder 
  CROT_ENCZ       = 0x5e,   //    External encoder
...
typedef enum {
  ...
  initrotateCan   = 3,   // 4 - x y (y 0.1 deg) (0.0-359.9)
  rotatePillbox   = 4,   // 2
  ctrlPump        = 6,   // 1
  ctrlLED         = 7,   // 72
  ctrlGate        = 8,   // 1
  readAllLimits   = 9,   // send 7
  readAllSensors  = 10,   // 20?
  readNFC         = 11,   // 4
  checkMachineStatus = 14,
  ctriticalError  = 16,
  CtrlStpX = 17,
  CtrlStpZ = 18,
  rotateLimCan    = 19,
	
  PressureSensor  = 20,   // 4 - x z dist in mm?
...
extern func_t func1[funcListLen];
extern uint32_t checktime;
extern uint8_t ARM_Z_dir, ARM_X_dir, isLimit_y,ismove_pill, CDoor,ispick, isZero_y, isZero_p, pnpX, pnpZ, lockcnt,depthsense;
extern uint8_t dispense_check;
