...
uint8_t VL53L0X_init(VL53L0X* dev)
{
  ...
  if (!VL53L0X_getSpadInfo(dev, &spad_count, &spad_type_is_aperture)) { return 0; }
 ...
 VL53L0X_setMeasurementTimingBudget(dev, dev->measurement_timing_budget_us);
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x40)) { return 0; }
  ...
  if (!VL53L0X_performSingleRefCalibration(dev, 0x00)) { return 0; }
...
void VL53L0X_writeReg(VL53L0X* dev, uint8_t reg, uint8_t value)
{
	...
	buf[0] = reg;
	buf[1] = value;
	dev->last_status = HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 2, 10);
}
...
void VL53L0X_writeReg16Bit(VL53L0X* dev, uint8_t reg, uint16_t value)
{
	...
	dev->last_status = HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 3, 10);
}
...
void VL53L0X_writeReg32Bit(VL53L0X* dev, uint8_t reg, uint32_t value)
{
	...
	dev->last_status = HAL_I2C_Master_Transmit(&hi2c2, dev->address, buf, 5, 10);
}
...
{
  uint8_t value;
  HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 10);
  dev->last_status = HAL_I2C_Master_Receive(&hi2c2, dev->address, &value, 1, 10);
  return value;
}
...
uint16_t VL53L0X_readReg16Bit(VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[2];
  HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 10);
  dev->last_status = HAL_I2C_Master_Receive(&hi2c2,dev->address, buf, 2, 10);
...
uint32_t VL53L0X_readReg32Bit(VL53L0X* dev, uint8_t reg)
{
  ...
  uint8_t buf[4];
  HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 10);
  dev->last_status = HAL_I2C_Master_Receive(&hi2c2,dev->address, buf, 4, 10);
...
{
	HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 10);
	dev->last_status = HAL_I2C_Master_Transmit(&hi2c2, dev->address, src, count, 10);
}
...
{
	HAL_I2C_Master_Transmit(&hi2c2, dev->address, &reg, 1, 10);
	dev->last_status = HAL_I2C_Master_Receive(&hi2c2, dev->address, dst, count, 10);;
}
...
uint8_t VL53L0X_setSignalRateLimit(VL53L0X* dev, float limit_Mcps)
{
...
uint8_t VL53L0X_setMeasurementTimingBudget(VL53L0X* dev, uint32_t budget_us)
{
...
uint8_t VL53L0X_setVcselPulsePeriod(VL53L0X* dev, enum VL53L0X_vcselPeriodType type, uint8_t period_pclks)
{
...
uint8_t VL53L0X_getVcselPulsePeriod(VL53L0X* dev, enum VL53L0X_vcselPeriodType type)
{
  if (type == VcselPeriodPreRange)
  {
...
// timeoutOccurred()?
uint8_t VL53L0X_timeoutOccurred(VL53L0X* dev)
{
  uint8_t tmp = dev->did_timeout;
  dev->did_timeout = 0;
  return tmp;
}
...
uint8_t VL53L0X_getSpadInfo(VL53L0X* dev, uint8_t * count, uint8_t * type_is_aperture)
{
  uint8_t tmp;
  ...
  {
    if (VL53L0X_checkTimeoutExpired(dev)) { return 0; }
  }
...
void VL53L0X_getSequenceStepEnables(VL53L0X* dev, struct VL53L0X_SequenceStepEnables* enables)
{
  uint8_t sequence_config = VL53L0X_readReg(dev,  SYSTEM_SEQUENCE_CONFIG);
...
// intermediate values
void VL53L0X_getSequenceStepTimeouts(VL53L0X* dev, struct VL53L0X_SequenceStepEnables* enables, struct VL53L0X_SequenceStepTimeouts* timeouts)
{
...
uint8_t VL53L0X_performSingleRefCalibration(VL53L0X* dev, uint8_t vhv_init_byte)
{
...
void VL53L0X_startTimeout(VL53L0X* dev){
	dev->timeout_start_ms = 5000;
}

uint8_t VL53L0X_checkTimeoutExpired(VL53L0X* dev){
	return (dev->io_timeout > 0 && (5000 - dev->timeout_start_ms) > dev->io_timeout);
}
