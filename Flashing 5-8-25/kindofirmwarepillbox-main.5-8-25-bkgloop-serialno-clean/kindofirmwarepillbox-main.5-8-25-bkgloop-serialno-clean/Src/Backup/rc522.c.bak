...
void SPI1_WriteReg(uint8_t address, uint8_t value) {
	if(nfcid==1){
		...
		cs2_reset();   //need to change
		SPI1SendByte(address);
		SPI1SendByte(value);
		cs2_set();				//need to change
...
uint8_t nfcRead(void) {
	//HAL_GPIO_WritePin(RFID_NRST_GPIO_Port, RFID_NRST_Pin,0);
	//HAL_Delay(10);
	int k=10;
	MFRC522_Init();
  while(k--){
       ...
       /* */for (i=0; i<4; i++) if (lastID[i] != str[i]) j = 1;                // Repeat test
        ...
        if (j && en) {
          ...
          for (i=0; i<8; i++) if (txBuffer[9+i] != memID[i]) ok = 0;
				...
				printf("MFRC522_Request 2 fail\n");
				if( nfcid==2) dataStream[PILLBX_HALL]=0;
			}
    }else {
			 //MFRC522_Init();
			//printf("nothing\n");
			if( nfcid==2)dataStream[PILLBX_HALL]=0;
				...
				dataStream[NFC_DATA]=0;
				dataStream[NFC_DATA+1]=0; 
				dataStream[NFC_DATA+2]=0;
				dataStream[NFC_DATA+3]=0;
